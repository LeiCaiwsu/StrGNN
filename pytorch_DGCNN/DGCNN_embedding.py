...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, window=5, batch=50, latent_dim=[32, 32, 32, 1], k=30, conv1d_channels=[16, 32], conv1d_kws=[0, 5], conv1d_activation='ReLU'):
        ...
        #self.conv1d_params3 = nn.Conv1d(conv1d_channels[-1], conv1d_channels[-1], window, window)
        ...
        #self.lstm = nn.RNN(self.dense_dim, 256, batch_first=True)
    ...
    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, e2n_sp, subg_sp, graph_sizes, node_degs):
        ...
        conv1d_res = self.conv1d_activation(conv1d_res)
        #conv1d_res = conv1d_res.view(int(len(graph_sizes)/self.window), conv1d_res.shape[1], -1)
        conv1d_res = conv1d_res.view(int(len(graph_sizes)/self.window), 32, -1)
        conv1d_res = self.conv1d_params3(conv1d_res)
        conv1d_res = self.conv1d_activation(conv1d_res)
        conv1d_res = conv1d_res.view(int(len(graph_sizes)/self.window), self.window, -1)

        #h0 = Variable(torch.zeros(1, conv1d_res.shape[1], conv1d_res.shape[2])).cuda()



        _,conv1d_res = self.lstm(conv1d_res)
        #conv1d_res = conv1d_res[-1]




        to_dense = conv1d_res.view(int(len(graph_sizes)/self.window), -1)

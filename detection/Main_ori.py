...
if args.train_name is None:
    ...
    net = np.load('data_ori/'+args.graph)

    if False:
        net_ = net.toarray()
        assert(np.allclose(net_, net_.T, atol=1e-8))
    #Sample train and test links
    f = np.load('data_ori/'+args.split+'.npz')
...
if not path.exists('data_ori/'+args.split+'h'+str(args.hop)):
    ...
    with open('data_ori/'+args.split+'h'+str(args.hop), 'wb') as f:
        pickle.dump([train_graphs, test_graphs, max_n_label], f, protocol=4)
else:
    with open('data_ori/'+args.split+'h'+str(args.hop), 'rb') as f:
